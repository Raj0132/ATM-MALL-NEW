...
import { Search, Download, RefreshCw, Users, AlertCircle, Phone, Mail, Calendar, MessageSquare } from 'lucide-react';
...
export default function LeadsPage() {
  ...
  const handleExport = () => {
    const filename = `atm-mall-leads-${new Date().toISOString().slice(0, 10)}`;
    exportToCsv(filename, filteredLeads, CSV_COLUMNS);
  };
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-xs text-slate-400">
            Total submissions: <strong className="text-amber-400 font-semibold">{leads.length}</strong>
            {search && filteredLeads.length !== leads.length && (
              <span> · Filtered: <strong className="text-slate-200">{filteredLeads.length}</strong></span>
            )}
          </p>
        </div>
      ...
      <div className="overflow-hidden rounded-xl border border-slate-800 bg-slate-900 shadow-sm">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20 text-slate-400">
            ...
            <p className="text-xs">Loading inquiries from Supabase...</p>
          </div>
        ) : filteredLeads.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-center text-slate-400">
            <Users className="h-10 w-10 text-slate-600 mb-3" />
            <h3 className="text-sm font-semibold text-slate-200">
              {search ? 'No matching leads found' : 'No inquiries yet'}
            </h3>
            <p className="mt-1 text-xs text-slate-500 max-w-xs">
              {search
                ? `No submissions matched "${search}". Try searching with different keywords.`
            ...
            <table className="w-full text-left text-xs text-slate-300">
              <thead className="border-b border-slate-800 bg-slate-950/70 text-[11px] uppercase tracking-wider text-slate-400 font-semibold">
                <tr>
                  ...
                  <th scope="col" className="px-4 py-3.5 whitespace-nowrap">Submitted Date</th>
                </tr>
              </thead>
                      ...
                      <div className="space-y-1">
                        {lead.email && (
                          <div className="flex items-center gap-1.5 text-slate-300">
                            ...
                            <a href={`mailto:${lead.email}`} className="hover:text-amber-400 transition-colors">
                              {lead.email}
                            </a>
                          </div>
                        )}
